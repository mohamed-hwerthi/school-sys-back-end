...
export default function VitrineModernLanding({ config, announcements, gallery }: Props) {
  return (
...
function ModernHero({ config }: { config: VitrineConfig }) {
  ...
  return (
    <section ref={ref} className="relative min-h-[100svh] overflow-hidden bg-[#0F172A]">
      ...
      {config.heroImageUrl && (
        <div className="absolute inset-0">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-b from-[#0F172A]/85 via-[#0F172A]/90 to-[#0F172A]" />
        </div>
      )}

      {/* gradient mesh blobs */}
      <div
        className="pointer-events-none absolute -top-40 -left-40 h-[600px] w-[600px] rounded-full blur-3xl opacity-40"
        style={{ background: `radial-gradient(circle, ${config.primaryColor}66 0%, transparent 70%)` }}
      />
      <div
        className="pointer-events-none absolute -bottom-40 -right-40 h-[700px] w-[700px] rounded-full blur-3xl opacity-30"
        style={{ background: `radial-gradient(circle, ${config.accentColor}66 0%, transparent 70%)` }}
      />
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_50%_120%,rgba(255,255,255,0.06),transparent)]" />

      <motion.div
        style={{ y, opacity }}
        className="relative mx-auto flex min-h-[100svh] max-w-7xl flex-col justify-center px-6 pt-32 pb-20 lg:px-8"
      >
        <motion.div
        ...
        <h1 className="mt-8 max-w-4xl font-heading text-5xl font-black leading-[1.05] tracking-tight text-white sm:text-6xl lg:text-7xl xl:text-[5.5rem]">
        ...
        >
          <MagneticButton
            href="#contact"
            className="group inline-flex items-center gap-2 rounded-full px-7 py-4 text-sm font-semibold text-white shadow-2xl"
            style={{ backgroundColor: config.accentColor }}
          >
            Nous contacter
          ...
          <a
            href="#about"
            className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/5 px-7 py-4 text-sm font-semibold text-white backdrop-blur transition-colors hover:bg-white/10"
          >
            Decouvrir l'ecole
          </a>
        </motion.div>
        ...
        <motion.div
          ...
          className="mt-20 grid grid-cols-1 gap-3 sm:grid-cols-3 sm:gap-6"
        >
          {[
            { icon: Phone, label: "Telephone", value: config.contactPhone, href: config.contactPhone ? `tel:${config.contactPhone}` : undefined },
            ...
            const inner = (
              <div className="group flex items-center gap-4 rounded-2xl border border-white/10 bg-white/5 p-4 backdrop-blur transition-colors hover:bg-white/10">
                <div
                ...
                <div className="min-w-0">
                  <p className="text-[10px] font-semibold uppercase tracking-widest text-white/50">{item.label}</p>
...
function TrustStrip({ config }: { config: VitrineConfig }) {
  const items = [
    { icon: GraduationCap, to: 1500, suffix: "+", label: "Eleves formes" },
    { icon: Users, to: 60, suffix: "+", label: "Enseignants" },
    { icon: BookOpen, to: 20, suffix: "+", label: "Programmes" },
    { icon: Award, to: 25, suffix: "", label: "Annees d'expertise" },
  ];
      ...
      <div className="mx-auto grid max-w-6xl gap-16 lg:grid-cols-2 lg:items-center">
        ...
        >
          ...
          <h2 className="mt-4 font-heading text-4xl font-bold leading-tight tracking-tight text-zinc-900 sm:text-5xl">
            Une education exigeante,<br />
            <span style={{ color: config.primaryColor }}>tournee vers l'avenir.</span>
          </h2>
          <p className="mt-6 text-lg leading-relaxed text-zinc-600">
            {config.slogan || "Nous accompagnons chaque eleve avec rigueur et bienveillance, dans un environnement propice a l'epanouissement et a l'excellence academique."}
          </p>
          {config.metaDescription && (
          ...
          <div className="mt-8 space-y-3">
            {[
              "Suivi individualise de chaque eleve",
              "Equipe pedagogique experimentee",
              "Equipements modernes et numeriques",
              "Activites parascolaires riches",
            ].map((item) => (
          ...
          <div className="absolute -bottom-6 -left-6 rounded-2xl bg-white p-5 shadow-2xl ring-1 ring-zinc-200/50">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-zinc-500">Taux de reussite</p>
              </div>
            </div>
          </div>
        </motion.div>
...
function ValuesSection({ config }: { config: VitrineConfig }) {
  const values = [
    { icon: GraduationCap, title: "Excellence academique", text: "Un cursus exigeant aligne sur les meilleurs standards." },
    { icon: Heart, title: "Bienveillance", text: "Un cadre humain, a l'ecoute et respectueux." },
    { icon: Sparkles, title: "Innovation", text: "Des methodes pedagogiques modernes, numeriques et engageantes." },
  ];
...
function ProgramsSection({ config }: { config: VitrineConfig }) {
  const programs = [
    { title: "Maternelle", level: "3 - 5 ans", text: "Eveil, sociabilisation et premiers apprentissages dans un cadre chaleureux." },
    { title: "Primaire", level: "6 - 11 ans", text: "Acquisition des fondamentaux : lecture, ecriture, calcul et raisonnement." },
    { title: "College", level: "12 - 15 ans", text: "Approfondissement, autonomie et orientation pour preparer l'avenir." },
  ];
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {items.map((a, i) => (
            ...
            >
              ...
              {a.body && (
                <p className="mt-3 line-clamp-3 text-sm leading-relaxed text-zinc-600">{a.body}</p>
              )}
            </motion.article>
          ))}
        </div>
      ...
      <div className="mx-auto max-w-4xl">
        ...
        >
          ...
          <blockquote className="mt-6 font-heading text-2xl font-medium leading-snug text-zinc-900 sm:text-3xl">
            « Une equipe a l'ecoute, des methodes modernes, et surtout des enfants epanouis qui aiment venir a l'ecole. Nous sommes ravis. »
          </blockquote>
          <div className="mt-8 flex items-center gap-4">
            ...
            <div>
              <p className="font-semibold text-zinc-900">Salma B.</p>
              <p className="text-sm text-zinc-500">Parent d'eleve</p>
            </div>
          </div>
        </motion.div>
      </div>
...
function ContactShowcase({ config }: { config: VitrineConfig }) {
  const ref = useRef<HTMLDivElement>(null);
  const inView = useInView(ref, { once: true, margin: "-80px" });
          ...
          >
            <div className="relative aspect-[4/3] w-full lg:aspect-auto lg:h-full lg:min-h-[480px]">
              <iframe
                ...
                loading="lazy"
                src={`https://www.openstreetmap.org/export/embed.html?bbox=-7.6,33.56,-7.58,33.58&layer=mapnik&marker=33.5731,-7.5898`}
              />
          ...
          <div className="relative grid gap-8 lg:grid-cols-2 lg:items-center">
            <div>
              ...
              <h2 className="mt-4 font-heading text-4xl font-bold leading-tight text-white sm:text-5xl">
                Prets a rejoindre<br />notre famille ?
              </h2>
              <p className="mt-5 max-w-md text-lg text-white/80">
                Pre-inscrivez votre enfant en ligne en quelques minutes. Notre equipe vous contactera rapidement.
              </p>
            </div>
